...
sed -e "s?__BUNDLEVERSION__?${XCS_INTEGRATION_NUMBER}?" DriveWireAppcast.xml | sed -e "s|__SIGNATURE__|$signature|" | sed -e "s?__LENGTH__?$length?" | sed -e "s?__VERSION__?$marketingVersion?" | sed -e "s?__RELEASEDATE__?$releaseDate?" | tr -d '\n' > /tmp/DriveWireAppcast.xml
